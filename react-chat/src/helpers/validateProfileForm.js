function validateProfileForm(values) {
  ...
  if (
    values.first_name &&
    (values.first_name.length < 2 || values.first_name.length > 150)
  ) {
    errors.first_name = 'Имя должно быть от 2 до 150 символов';
  }

  if (
    values.last_name &&
    (values.last_name.length < 2 || values.last_name.length > 150)
  ) {
    errors.last_name = 'Фамилия должна быть от 2 до 150 символов';
  }
